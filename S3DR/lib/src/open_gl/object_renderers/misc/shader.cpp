...
Shader::Shader(const std::string &shader_string, GLenum shader_type, const std::string &shader_name) : shader_obj(new GLuint)
{
   ...
   *shader_obj = glCreateShader(shader_type);
   ...
   if (status == GL_FALSE)
   {
      std::string error = std::string("*** Failed to compile shader: ") + shader_name + "\n";
