    ...
    , opr(nullptr)
{
    ...
    camera.reset(new Camera(glm::vec3(0.0f, 0.0f, 15.0f), glm::vec3(0.0, 0.0, -1.0), glm::vec3(0.0, 1.0, 0.0)));
...
void View::Redraw()
{
    if(view_adapter)
    {
        view_adapter->Render(camera->CameraMatrixRef(), projection->ProjectionMatrixRef());
    }
}
