...
uniform sampler2D shell_texture;
...
vec4 ComputeLighting(in Light lightData, in vec4 diffuseColor)
{
    vec3 lightDir;
    vec4 lightIntensity;
...
void main()
{
    vec4 diffuseColor = texture(shell_texture, textureCord);
    vec4 accumLighting = diffuseColor * ambientIntensity;

    for(int light = 0; light < numberOfLights; light++)
    {
        accumLighting += ComputeLighting(lights[light], diffuseColor);
    }
