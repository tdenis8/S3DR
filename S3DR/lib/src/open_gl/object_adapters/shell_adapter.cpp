...
ShellAdapter::ShellAdapter(ShellObject& shell_object) : SceneObjectAdapter(shell_object), shell_object_(shell_object)
{
...
{
    const ShellData& shell_data = shell_object_.GetShellData();
...
{
    glBindVertexArray(vao_object_);
    const std::vector<ShellEntry>& entries = shell_object_.GetShellData().entries;
...
{
    glBindVertexArray(vao_object_);
    set_scene_object_key(shell_object_.GetKey());
    const std::vector<ShellEntry>& entries = shell_object_.GetShellData().entries;
    int entry = 0;
    for(auto iter = entries.begin(); iter != entries.end(); ++iter)
    {
